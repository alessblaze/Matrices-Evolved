 ...
 *
 * High-Performance Rust Stream Change Cache Implementation for Synapse
 * 
 * Features:
 * - O(log n) stream position operations with BTreeMap
 * - O(1) entity lookup with HashMap
 * - Atomic metrics for thread-safe performance tracking
 * - Memory-efficient Arc<str> for entity deduplication
 * - Configurable cache size with dynamic resizing
 * - Comprehensive Python bindings via PyO3
 */
