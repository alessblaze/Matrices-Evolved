...
int fast_double_to_string(double f, char *result)
{
    ...
    if (f == 0.0)
    {
        ...
        result[2] = '0';
        if (debug_enabled)
            std::cout << "DEBUG fast_double_to_string: fast path 0.0 -> \"0.0\"" << std::endl;
        return 3;
    }
    if (f == 1.0)
    {
        ...
        result[2] = '0';
        if (debug_enabled)
            std::cout << "DEBUG fast_double_to_string: fast path 1.0 -> \"1.0\"" << std::endl;
        return 3;
    }
                ...
                if (s.size() >= 64)
                {
                    ...
                    for (; i + 64 <= len; i += 64)
                    {
                        ...
                        if (__builtin_expect(_mm256_testz_si256(escape_mask1, escape_mask1) &&
                                                 _mm256_testz_si256(escape_mask2, escape_mask2),
                                             1))
                        {
                        ...
                        else
                        {
                            ...
                            while (mask)
                            {
                                ...
                                if (good_len)
                                    write_raw_unsafe(data + base, good_len);
                                ...
                                case '\\':
                                    ...
                                    break;
                                case '\b':
                                    write_raw_unsafe("\\b", 2);
                                    break;
                                case '\f':
                                    write_raw_unsafe("\\f", 2);
                                    break;
                                case '\n':
                                    write_raw_unsafe("\\n", 2);
                                    break;
                                case '\r':
                                    write_raw_unsafe("\\r", 2);
                                    break;
                                case '\t':
                                    write_raw_unsafe("\\t", 2);
                                    break;
                            ...
                            if (base < i + 32)
                                write_raw_unsafe(data + base, (i + 32) - base);
                        }
                        ...
                        else
                        {
                            ...
                            if (base < i + 64)
                                write_raw_unsafe(data + base, (i + 64) - base);
                        }
                    }
